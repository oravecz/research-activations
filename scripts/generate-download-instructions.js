...
const eventsData = JSON.parse(
  fs.readFileSync(path.join(__dirname, '..', 'public', 'events', 'details.json'), 'utf-8')
);
...
function generateInstructions(event) {
  const brandSlug = slugify(event.brand);
  const imagesNeeded = 9 - (event.imageInventory?.totalImages || 1);

  // Extract a shorter activation description (first sentence or up to 80 chars)
  let activationSnippet = event.activation.split('.')[0];
  if (activationSnippet.length > 80) {
    activationSnippet = activationSnippet.substring(0, 80);
  }

  const instructions = `═══════════════════════════════════════════════════════════════════════════════
PLAYWRIGHT IMAGE COLLECTION INSTRUCTIONS
Event ${event.id}: ${event.brand} - ${event.title}
Need ${imagesNeeded} more images
═══════════════════════════════════════════════════════════════════════════════
...
PRIMARY (Most specific - TRY FIRST):
  1. "${event.brand} ${event.title} activation photos"
  2. "${event.brand} ${event.title} event images ${event.date}"

SECONDARY (Look for event coverage on news/trade sites):
  1. site:retailtouchpoints.com "${event.brand} ${event.title}"
  2. site:parasolprojects.com "${event.brand} ${event.title}"
  3. "${event.brand} pop-up" event 2024 photographer

TERTIARY (Broader brand activation searches):
  1. "${event.brand} store activation ${event.date.split('-')[0]}"
  2. "${event.brand} retail marketing event"
  3. "${event.brand} in-store experience"
...
3. Save to: public/events/${brandSlug}/event-${event.id}/images/
4. Name as: image-[N].png or image-[N].webp (where N is next available number)
5. Target: ${imagesNeeded} images (total 9 with logo)
...
EXAMPLE NODE SCRIPT:
  ...
  const filePath = 'public/events/${brandSlug}/event-${event.id}/images/image-1.png';
...
1. Verify all images saved to: public/events/${brandSlug}/event-${event.id}/images/
...
eventsData.events.forEach(event => {
  const brandSlug = slugify(event.brand);
  const imagesDir = path.join(__dirname, '..', 'public', 'events', brandSlug, `event-${event.id}`, 'images');
  const instructionsPath = path.join(imagesDir, 'DOWNLOAD_INSTRUCTIONS.md');
  ...
  // Overwrite existing instructions to ensure consistency
  // (Comment out to skip existing files)
  // if (fs.existsSync(instructionsPath)) {
  //   console.log(`⊘ Skipping Event ${event.id} (${event.brand}): Instructions already exist`);
  //   skipped++;
  //   return;
  // }

  // Generate and write instructions
  const instructions = generateInstructions(event);
  fs.writeFileSync(instructionsPath, instructions, 'utf-8');
  console.log(`✓ Created instructions for Event ${event.id}: ${event.brand} - ${event.title}`);
  created++;
});

console.log(`\n─────────────────────────────────────────────────`);
console.log(`✓ Created: ${created} instruction files`);
